...
An payment gateway system that meets all three factors above with a high traffic, billions of users is a difficult
problem.
...
is only necessary to handle the cache in the most traditional way. When the cache is invalidate, a thread will read in
...
with the master-card system. The authen, author will be done with Auth, Author Service. </br>
...
A large project like Master Card will have many different providers: banks, payment gateways, e-wallets. All this
...
listen to kafka and push important information to the Elastic Search Cluster. Elastctic Search Cluster is a search
...
Search Service will get more at Wearhouse Service to get more statistical information for merchants. Warehouse Service
...
appropriate events back into kafka. Hadoop is the data source for data enginer and marchine learning, AI works. I'm not
...
Before merchant wants to checkout, Order Talking Service will talk to RiskService to check the risk. If it fails, it is
...
url, this url points to the url that marchant has registered if the payment is successful. </br>

When the payment fails, the action is the same as the successful payment but with thestatus "error_checkout". The
...
The Error Checking Service will run in the background to check for all irregularities, orders that have not been
statused for too long and notify kafka, process and notify relevant parties. With financial services and complex
process, an Error Checkking Servcie is needed to detect and correct deviations early. </br>
...
maintenance hours, setup time notify, system timeout... Absolute ACID information such as privated_key, .... will be
